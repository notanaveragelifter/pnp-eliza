...
export class TwitterInteractionClient {
   ...
   */
  async start() {
    // Check if Twitter interactions are enabled
    const interactionEnabled = parseBooleanFromText(
      this.state?.TWITTER_INTERACTION_ENABLE ||
        this.runtime.getSetting('TWITTER_INTERACTION_ENABLE')
    );

    if (!interactionEnabled) {
      logger.log(
        'Twitter interaction client: Automatic interactions disabled. Set TWITTER_INTERACTION_ENABLE to true to enable.'
      );
      return;
    }

    logger.log('Twitter interaction client: Automatic interactions enabled');
