...
export class TwitterPostClient {
  ...
  async start() {
    ...
    // Check if automatic post generation is enabled
    const enablePostGeneration = parseBooleanFromText(
      this.state?.TWITTER_ENABLE_POST_GENERATION ||
        this.runtime.getSetting('TWITTER_ENABLE_POST_GENERATION')
    );

    if (!enablePostGeneration) {
      logger.log(
        'Automatic tweet generation is disabled. Set TWITTER_ENABLE_POST_GENERATION to true to enable.'
      );
      return;
    }
